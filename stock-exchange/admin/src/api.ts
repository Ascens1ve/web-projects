import { baseURL, IAuth, IBroker, IStart, IStocks, IUser, StocksCompanies } from './interfaces';


export const loginByAlias = async (alias: string, password: string)
: Promise<IAuth> => {
    let response;
    try {
...
export const sendChoosedCompanies = async (companies: StocksCompanies[])
: Promise<{ status: string; message: string; data: StocksCompanies[] }> => {
    ...
    if (!response.ok) throw new Error((await response.json()).message);
    return response.json() as Promise<{ status: string; message: string; data: StocksCompanies[] }>;
};

export const getMinMaxDates = async (): Promise<{ min: string; max: string }> => {
    let response;
    try {
...
export const postBroker = async (data: { [key: string]: string; }): Promise<IBroker[]> => {
    let response;
    try {
