...
export const StocksPage = () => {
    ...
    const data = useSelector(
        useCallback(selectStocksByName(company), [company])
    );
    ...
    const processedData = useMemo(() =>
        data.map((d: IStocks) => ({ x: Date.parse(d.Date), y: parseDollar(d.Open) })), [data]);

    const {minY, maxY, dateLimits} = useMemo(() => {
            if (!data.length) return {minY: 0, maxY: 0, dateLimits: {min: 0, max: 0}};
            ...
            return {minY, maxY, dateLimits: {min: minX - 10*day, max: maxX + 10*day}};
    }, [data]);
            ...
            options: {
                ...
                plugins: {
                    zoom: {
                        ...
                        limits: { x: { min: dateLimits.min, max: dateLimits.max },
        ...
        <>
            ...
            <div className="main">
                <div className="companies">
                    <h2 className="companies__header">
                        Выберите компании для участия в торгах:
                    </h2>
                    <form
                        onSubmit={handleSubmit}
                        className="companies__checkboxes"
                    >
