...
>('stocks/get', async (name: string, thunkAPI) => {
    ...
    try {
        ...
        return thunkAPI.rejectWithValue(`Ошибка при получении акций ${name}: ${(error as Error).message}`);
    }
});
...
>('stocks/send-companies', async (_, thunkAPI) => {
    ...
    if (choosedCompanies.length === 0) return thunkAPI.rejectWithValue('Некорректные компании');
    try {
        await sendChoosedCompanies(choosedCompanies);
        return 0;
    } catch (error) {
        return thunkAPI.rejectWithValue(`Ошибка при загрузке выбранных компаний: ${(error as Error).message}`);
    }
});
...
export const sendStart = createAsyncThunk<string, void, { state: { stocks: StocksState}, rejectValue: string }>
('stocks/send-start', async (_, thunkAPI) => {
    const { bidSpeed, choosedCompanies, startDate } = thunkAPI.getState().stocks;
    if (!startDate || !bidSpeed) return thunkAPI.rejectWithValue('Ошибка при старте торгов');
    try {
        return await setStart({currentDate: startDate, bidSpeed, choosedCompanies });
    }
    catch (error) {
...
export const sendPause = createAsyncThunk<string, void, { rejectValue: string }>
...
export const sendResume = createAsyncThunk<string, void, { rejectValue: string }>
...
('stocks/send-stop', async (_, thunkAPI) => {
    try {
...
export const stocksSlice = createSlice({
    ...
    extraReducers: (builder) => {
        builder
            ...
            .addCase(
                postChoosedCompanies.rejected,
                (state: StocksState, action) => {
                    state.isSending = false;
                    state.error = action.payload ?? 'Ошибка при загрузке выбранных компаний';
                },
            )
            .addCase(
                postChoosedCompanies.fulfilled,
                (state: StocksState) => {
                    state.isSending = false;
                    state.error = '';
                },
            ...
            .addCase(
                getStartEndDates.rejected,
                (state: StocksState, action) => {
                    state.isSending = false;
                    state.error = action.payload ?? 'Ошибка при получении начальной и конечной даты торгов';
                },
            )
            .addCase(
...
export const {
    chooseCompanies, removeCompanies, setBidSpeed,
    setStartDate, setBidState, clearError } = stocksSlice.actions;
