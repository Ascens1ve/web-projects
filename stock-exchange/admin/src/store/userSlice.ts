...
>('user/login', async (data, thunkAPI) => {
    try {
        ...
        return thunkAPI.rejectWithValue(`Ошибка при входе: ${(error as Error).message}`);
    }
});
...
export const getUser = createAsyncThunk<IUser, void, { rejectValue: string }>(
    ...
    async (_, thunkAPI) => {
        try {
            ...
            return thunkAPI.rejectWithValue(`Ошибка при получении данных пользователя: ${(error as Error).message}`);
        }
    },
);
...
export const { authChecked, changeLang, logout, clearError } = userSlice.actions;
